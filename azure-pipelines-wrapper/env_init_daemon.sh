...
cd workspace
find . -maxdepth 2 -name "tmp.*" -type d -ctime +30 -delete &>> env_init_daemon.log

if (( "$(df -h | grep '% /home' | awk '{print$5}' | grep -Eo [0-9]*)" > "60"));then
    find . -maxdepth 2 -name "tmp.*" -type d -ctime +20 -delete &>> env_init_daemon.log
fi
...
for i in $(cat todo.list); do
    ...
    ./script.sh | sed "s/ms_checker.result: /ms_checker.result: $PR_NUMBER=/" | while IFS= read -r line; do echo "[$(date '+%FT%TZ')] $line" > tmp
    cat tmp
    echo $i >> done.list
    sleep 2
done